...
CRGB leds0[ NUM_LEDS_F + 1 ];
CRGB leds1[ NUM_LEDS_M + 1 ];
CRGB leds2[ NUM_LEDS_R + 1 ];
CRGB*  ledsF( leds0+1 ); 
CRGB*  ledsM( leds1+1 );
CRGB*  ledsR( leds2+1 );
...
void setup() {
 ...
 FastLED.addLeds<CHIPSET, LED_PIN_FRONT, COLOR_ORDER>(ledsF, numPanelLEDs[0]).setCorrection(TypicalSMD5050);
...
uint16_t XY( uint8_t a, uint8_t x, uint8_t y)
{
  ...
  //check if not a real pixel 
  if (j >= numPanelLEDs[a-1]) j = -1;
...
*/        
     ...
     if (x < arrayStart[1]) {
        ...
        ledsM[xyout]  = val;
        if (oled && (xyout < numPanelLEDs[1])) display.drawPixel(2*x+2, SCREEN_HEIGHT - y, SSD1306_WHITE);
      }
      else {
        xyout= XY(3, y, x - arrayStart[2]);
        ledsR[xyout]  = val;
        if (oled && (xyout < numPanelLEDs[2])) display.drawPixel(2*x+4, SCREEN_HEIGHT - y, SSD1306_WHITE);
      }
